var tandaTangan = (function () {
	...
	return {
		...
		capture: function () {
			...
			if (!context) {
				throw new Error("Gagal get Canvas 2d Context"); //still bug! (nvm it's done)
			}

			screenwidth = screen.width;

			//let's create condition for dynamic width
		...
		send: function () {
			if (empty == false) {
				...
				dataform.innerHTML =
					...
					sendemail +
					'"/>' +
					'<input type="email" name="replyemail" value="' +
					replyemail +
					'"/>' +
