                  ...
                  <div class="modal-dialog">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <table class="table  table-bordered table-hover table-td-valign-middle text-black" style="border-color: #d3d3d3">
                        <thead>
                                <input style="border-color: #d3d3d3; color: black" type="hidden" class="form-control" name="kontak_pelanggan_id2" id="kontak_pelanggan_id2" placeholder="Nama" value="" />
...
<script type="text/javascript">
    $(document).ready(function(){
                ...
                success : function(data){
                    ...
                    for(i=0; i<data.length; i++){
                        html += '<tr>'+
                                ...
                                '<td>'+
                                    '<a href="javascript:;" class="btn btn-info btn-xs item_edit" data="'+data[i].kontak_pelanggan_id+'"><i class="fas fa-pencil-alt" aria-hidden="true"></i></a>'+' '+
                                    '<a href="javascript:;" class="btn btn-danger btn-xs data_hapus" data="'+data[i].kontak_pelanggan_id+'"><i class="fas fa-trash-alt" aria-hidden="true"></i></a>'+
                                '</td>'+
                                '</tr>';
                    }
                    $('#show_data').html(html);
        ...
        $('#show_data').on('click','.item_edit',function(){
            ...
            $.ajax({
                type : "GET",
                url  : "<?php echo base_url('kontak_pelanggan/get_data')?>",
                dataType : "JSON",
                data : {id:id},
                success: function(data){
                    $.each(data,function(kontak_pelanggan_id,pelanggan_id,nama, title_jabatan, telepon){
                        $('#ModalaEdit').modal('show');
                        $('[name="kontak_pelanggan_id2"]').val(data.kontak_pelanggan_id);
                        $('[name="pelanggan_id2"]').val(data.pelanggan_id);
        ...
        $('#btn_update').on('click',function(){
            var kontak_pelanggan_id2=$('#kontak_pelanggan_id2').val();
            var pelanggan_id2=$('#pelanggan_id2').val();
            var nama2=$('#nama2').val();
            var title_jabatan2=$('#title_jabatan2').val();
            var telepon2=$('#telepon2').val();
            $.ajax({
                type : "POST",
                url  : "<?php echo base_url('kontak_pelanggan/update_data')?>",
                data : {
                    kontak_pelanggan_id2:kontak_pelanggan_id2,
                    pelanggan_id2:pelanggan_id2,
                    nama2:nama2,
                    title_jabatan2:title_jabatan2,
                    telepon2:telepon2
                },
                success: function(data){
                    $('[name="kontak_pelanggan_id2"]').val("");
                    $('[name="nama2"]').val("");
        ...
        $('#btn_hapus').on('click',function(){
            ...
            $.ajax({
            type : "POST",
            url  : "<?php echo base_url('kontak_pelanggan/hapus')?>",
            dataType : "JSON",
                    data : {kode: kode},
                    success: function(data){
